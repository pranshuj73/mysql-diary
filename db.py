...
def create_entry(mood: int, entry: str, dt: str):
    ...
    cur.execute(
        f'insert into diary(mood, entry, dt) values({mood}, "{entry}", "{dt}");'
    )
    db.commit()
...
def change_status(task, new_status):
    ...
    cur.execute(
        f"update todo set status={new_status} where title='{title}' and description='{description}' and priority={priority} and status={status};"
    )
    db.commit()
...
def del_task(task):
    ...
    title, description, priority, status = task
    cur.execute(
        f"delete from todo where title='{title}' and description='{description}' and priority={priority} and status={status};"
    )
    db.commit()
