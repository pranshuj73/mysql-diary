...
class Andy(Tk):
	def __init__(self):
		...
		self.frames = {}
		for frame_page in (Homescreen, DiaryFrame, DiaryCreateFrame, EntryListFrame, FutureEntryFrame, TodoFrame, TaskCreateFrame, TaskEditFrame):
...
class Homescreen(Frame):
	...
	def __init__(self, parent, controller):
		...
		title_label = Label(inner_frame, image=andy, text="Hii, my name is Andy, your personal diary and to-do app. ^_^", compound="top", pady=20)
		title_label.grid(row=0, column=0, columnspan=2, pady=(0,10))

		diary_img = ImageTk.PhotoImage(Image.open("assets/diary.png").resize((60, 60), Image.ANTIALIAS))
		diary_btn = Button(inner_frame, text="Diary", image=diary_img, anchor="center", height=100, width=100, compound="top", bd=0, command=lambda: controller.show_frame("DiaryFrame"), cursor="hand1")
		diary_btn.grid(row=1, column=0)

		todo_img = ImageTk.PhotoImage(Image.open("assets/todo.png").resize((60, 60), Image.ANTIALIAS))
		todo_btn = Button(inner_frame, text="To-Do", image=todo_img, anchor="center", height=100, width=100, compound="top", bd=0, command=lambda: self.load_todo_frame(), cursor="hand1")
...
class DiaryFrame(Frame):
	...
	def check_records(self, date):
		...
		if converted_date <= datetime.today():
			global entries
			...
			self.controller.show_frame("FutureEntryFrame")
...
class DiaryCreateFrame(Frame):
	...
	def __init__(self, parent, controller):
		...
		back_btn = Button(self, text="Back", image=back_img, anchor="center", compound="left", bd=0, padx=-10, command=lambda: controller.show_frame("DiaryFrame"), cursor="hand1")
		...
		save_img = ImageTk.PhotoImage(Image.open("assets/save.png").resize((40, 40), Image.ANTIALIAS))
		save_btn = Button(self, text="Save your new entry", image=save_img, anchor="center", compound="left", bd=0, command=lambda: self.save_entry(), cursor="hand1")
...
class EntryListFrame(Frame):
	...
	def __init__(self, parent, controller):
		...
		no_entry_img = ImageTk.PhotoImage(Image.open("assets/no_entry.png").resize((128, 128), Image.ANTIALIAS))

		back_btn = Button(self, text="Back", image=back_img, anchor="center", compound="left", bd=0, padx=-10, command=lambda: self.show_prev_frame(), cursor="hand1")
		back_btn.pack(pady=(0, 20))
	...
	def load_content(self, date):
		...
		if  entries:
			...
			create_btn = Button(inner_frame, text="Add another entry?", image=create_img, anchor="center", compound="left", bd=0, command=lambda: self.controller.show_frame("DiaryCreateFrame"), cursor="hand1").pack(pady=10)

		elif not entries:
			empty_msg = Label(inner_frame, text="No entries were created for this day", image=no_entry_img, anchor="center", compound="top", bd=0, padx=10, pady=10, wraplength=self.winfo_width()-50)
			empty_msg.pack(pady=50)

			create_btn = Button(inner_frame, text="Why not add an entry?", image=create_img, anchor="center", compound="left", bd=0, command=lambda: self.controller.show_frame("DiaryCreateFrame"), cursor="hand1").pack(pady=10)
...
class FutureEntryFrame(Frame):
	...
	def __init__(self, parent, controller):
		...
		crying_img = ImageTk.PhotoImage(Image.open("assets/crying.png").resize((128, 128), Image.ANTIALIAS))

		back_btn = Button(self, text="Back", image=back_img, anchor="center", compound="left", bd=0, padx=-10, command=lambda: self.controller.show_frame("DiaryFrame"), cursor="hand1")
		back_btn.pack(pady=(0, 20))

		cant_add_entries_msg = Label(self, text="Unfortunately, only those with time-travelling abilities\ncan see diary entries for the future!", image=crying_img, anchor="center", compound="top", bd=0, padx=10, pady=30, font=('Arial', 14))
		cant_add_entries_msg.pack(pady=50)
...
class TodoFrame(Frame):
	...
	def __init__(self, parent, controller):
		...
		title_label = Label(self, image=todo_img, text="Tasks", compound="left", padx=10, font=('Arial', 16))
		title_label.pack()
		...
		add_task_img = ImageTk.PhotoImage(Image.open("assets/todo_add_new.png").resize((45, 45), Image.ANTIALIAS))
		add_task_btn = Button(self, text="Add new task", image=add_task_img, anchor="center", compound="left", bd=0, command=lambda: controller.show_frame("TaskCreateFrame"), cursor="hand1")
		add_task_btn.pack()
	...
	def load_tasks(self):
		...
		if self.incomplete_tasks:
			...
			for task in self.incomplete_tasks:
				...
				task_btn = Button(task_frame, text=title, bg=priority_colors[priority-1], bd=0, width=25, wraplength=400, justify="left", anchor="w", pady=5, command=lambda task=task: self.get_task(task), cursor="hand1")
			...
			if self.completed_tasks:
				...
				for task in self.completed_tasks:
					title, description, priority, status = task
					task_btn = Button(task_frame, text=title, image=done_icon, compound="left", bd=0, width=200, wraplength=400, justify="left", anchor="w", pady=5, command=lambda task=task: self.get_task(task), cursor="hand1")
					task_btn.pack(expand=True, padx=(50,0))

		else:
			global no_task_img
			no_task_img = ImageTk.PhotoImage(Image.open("assets/no_task.png").resize((128, 128), Image.ANTIALIAS))
			all_complete = Label(task_frame, text="You're all caught up!", image=no_task_img, compound="top", font=('Arial', 16), pady=20).pack(padx=(50,0), pady=10)
...
class TaskCreateFrame(Frame):
	...
	def __init__(self, parent, controller):
		...
		back_btn = Button(self, text="Back", image=back_img, anchor="center", compound="left", bd=0, padx=-10, command=lambda: self.load_todo_frame(), cursor="hand1")
		...
		self.priority_1_btn = Button(self, text="Important & Urgent\n[Do it now]", bd=0, bg="#80FDFF", fg="gray15", highlightbackground="#80FDFF", disabledforeground="black", pady=10, padx=5, command=lambda: self.disable_self(1), cursor="hand1")
		self.priority_2_btn = Button(self, text="Important but not Urgent\n[Schedule a time to do it]", bd=0, bg="#90EE90", fg="gray15", highlightbackground="#90EE90", disabledforeground="black", pady=10, padx=5, command=lambda: self.disable_self(2), cursor="hand1")
		self.priority_3_btn = Button(self, text="Urgent but not Important\n[What can do it for you?]", bd=0, bg="#FF9380", fg="gray15", highlightbackground="#FF9380", disabledforeground="black", pady=10, padx=5, command=lambda: self.disable_self(3), cursor="hand1")
		self.priority_4_btn = Button(self, text="Neither Important nor Urgent\n[Eliminate it]", bd=0, bg="#A5BCD9", fg="gray15", highlightbackground="#A5BCD9", disabledforeground="black", pady=10, padx=5, command=lambda: self.disable_self(4), cursor="hand1")
		...
		save_btn = Button(self, text="Save your task", image=save_img, anchor="center", compound="left", bd=0, command=lambda: self.save_entry(), cursor="hand1")
...
class TaskEditFrame(Frame):
	...
	def __init__(self, parent, controller):
		...
		delete_icon = ImageTk.PhotoImage(Image.open("assets/delete.png").resize((45, 45), Image.ANTIALIAS))
		completed_icon = ImageTk.PhotoImage(Image.open("assets/completed.png").resize((45, 45), Image.ANTIALIAS))	
		incomplete_icon = ImageTk.PhotoImage(Image.open("assets/incomplete.png").resize((45, 45), Image.ANTIALIAS))
		...
		self.change_status_btn = Button(self, bd=0, cursor="hand1")
		self.delete_btn = Button(self, text="Delete", bd=0, image=delete_icon, compound="left", anchor="center", cursor="hand1")
